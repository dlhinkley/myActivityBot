...
QUnit.test( "maproom.calcScanRoute 200x200", function( assert ) {
    ...
    // Walls around robot
	assert.equal( mapGrid.cellEast( mapGrid.cellEast(robotCell)).x , 200, "Cell to east east of robot x");
	assert.equal( mapGrid.cellEast( mapGrid.cellEast(robotCell)).y , 100, "Cell to east east of robot y");
	assert.ok( mapGrid.cellEast( mapGrid.cellEast(robotCell)).isWall() , "Cell to east east of robot wall");

	assert.equal( mapGrid.cellWest( mapGrid.cellWest(robotCell)).x , 0, "Cell to west west of robot x");
	assert.equal( mapGrid.cellWest( mapGrid.cellWest(robotCell)).y , 100, "Cell to west west of robot y");
	assert.ok( mapGrid.cellWest( mapGrid.cellWest(robotCell)).isWall() , "Cell to west west of robot wall");
